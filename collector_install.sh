...
# Optional values
K8S_COLLECTOR_DEFAULT_HELM_REPO="oci://tanzu-platform.packages.broadcom.com/adie/helm-charts/k8s-collector"
PXLSCRIPTS_DEFAULT_HELM_REPO="oci://tanzu-platform.packages.broadcom.com/adie/helm-charts/pxl-scripts"
TELEGRAF_DEFAULT_HELM_REPO="oci://tanzu-platform.packages.broadcom.com/adie/helm-charts/telegraf-collector"
...
VMW_DEFAULT_DEBUG=0
VMW_DEFAULT_DEPLOY_K8S_COLLECTOR=1
VMW_DEFAULT_DEPLOY_OBSERVABILITY=1
VMW_DEFAULT_DEPLOY_PIXIE=1
VMW_DEFAULT_DEPLOY_TELEGRAF=1
...
VMW_DEFAULT_LEMANS_ROUTE_NAME_CAS="cas-service-route"
VMW_DEFAULT_LEMANS_STREAM_NAME_EVENTS="k8s-events"
VMW_DEFAULT_LEMANS_STREAM_NAME_INVENTORY="k8s-inventory"
...
VMW_DEFAULT_NO_VERIFY_TLS="true"
...
VMW_COLLECTOR_VERSION_OVERRIDE="3.19.1"
...
VMW_DEPLOY_K8S_COLLECTOR="${VMW_DEPLOY_K8S_COLLECTOR:-"${VMW_DEFAULT_DEPLOY_K8S_COLLECTOR}"}"
VMW_DEPLOY_OBSERVABILITY="${VMW_DEPLOY_OBSERVABILITY:-"${VMW_DEFAULT_DEPLOY_OBSERVABILITY}"}"
VMW_DEPLOY_PIXIE="${VMW_DEPLOY_PIXIE:-"${VMW_DEFAULT_DEPLOY_PIXIE}"}"
VMW_DEPLOY_TELEGRAF="${VMW_DEPLOY_TELEGRAF:-"${VMW_DEFAULT_DEPLOY_TELEGRAF}"}"
...
VMW_LEMANS_ROUTE_NAME_CAS="${VMW_LEMANS_ROUTE_NAME_CAS:-"${VMW_DEFAULT_LEMANS_ROUTE_NAME_CAS}"}"
...
VMW_NO_VERIFY_TLS="${VMW_NO_VERIFY_TLS:-"${VMW_DEFAULT_NO_VERIFY_TLS}"}"
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      ...
      --label-namespaces)
        ...
        ;;
      --no-deploy-pixie)
        # Disable the deployment of Pixie for eBPF metrics collection (CPU/Memory/Disk)
        VMW_DEPLOY_PIXIE=0
        shift # past argument
        ;;
      --no-deploy-k8s-collector)
        # Disable the deployment of the Kubernetes Inventory collector
        VMW_DEPLOY_K8S_COLLECTOR=0
        shift # past argument
        ;;
      --no-deploy-telegraf)
        # Disable the deployment of telegraf used to forward Pixie metrics to TSS and
        # collect metrics from Spring applications.
        #
        # When disabled, this implicitly also disables the collection of Spring applications,
        # Pixie data, and Spring Inventory because these collectors send their information
        # via telegraf.
        VMW_DEPLOY_TELEGRAF=0
        shift # past argument
        ;;
      --no-deploy-observability)
        # Disable the deployment of the wavefront-operator and wavefront-cluster-collector
        VMW_DEPLOY_OBSERVABILITY=0
        shift # past argument
        ;;
      --no-app-metrics)
        # Disable the Spring Application metrics configuration in telegraf
        VMW_APP_METRICS=0
        shift # past argument
        ;;
      --no-spring-inventory)
        # Disable the Spring Inventory configuration in telegraf
      ...
      --inventory-stream-name)
        ...
        ;;
      --cas-route-name)
        VMW_LEMANS_ROUTE_NAME_CAS="$2"
        shift # past argument
        shift # past value
        ;;
      --verify-tls)
        VMW_NO_VERIFY_TLS="false"
        shift # past argument
        ;;
      --self-managed)
...
check_k8s_install_required_args() {
  ...
  if [[ -z "${VMW_COLLECTOR_CLIENT_SECRET:-}" ]]; then
    if [[ "${VMW_SELF_MANAGED}" == "true" ]] ; then
      # Use a placeholder
      VMW_COLLECTOR_CLIENT_SECRET="none"
    else 
...
print_work_to_do_wrapper() {
  if [[ $VMW_INSTALL_K8S -eq 1 && $VMW_DEPLOY_K8S_COLLECTOR -eq 1 ]]; then
  ...
  if [[ $VMW_INSTALL_TELEGRAF -eq 1 && $VMW_DEPLOY_TELEGRAF -eq 1 ]]; then
...
collector:
  extraEnvironmentVars:
    ...
    LEMANS_ROUTER_NAME_CAS: "${VMW_LEMANS_ROUTE_NAME_CAS}"
    NO_VERIFY_TLS: "${VMW_NO_VERIFY_TLS}"
...
install_telegraf() {
  if [[ $VMW_DEPLOY_TELEGRAF -eq 0 ]]; then
    ohai "Not deploying telegraf"
    return
  fi
...
install_pxl_scripts() {
  if [[ $VMW_DEPLOY_PIXIE -eq 0 ]]; then
    ohai "Not deploying Pixie PxL scripts"
    return
  fi
...
install_observability() {
  if [[ $VMW_DEPLOY_OBSERVABILITY -eq 0 ]]; then
    ohai "Not deploying observability"
    return
  fi
...
install_k8s_collector() {
  if [[ $VMW_DEPLOY_K8S_COLLECTOR -eq 0 ]]; then
    ohai "Not deploying Kubernetes collector"
    return
  fi
...
install_observability_config() {
  if [[ $VMW_DEPLOY_OBSERVABILITY -eq 0 ]]; then
    ohai "Not deploying observability config"
    return
  fi
